...
namespace GeometryAPI
{
    ...
    {
        private double radius;
        private Vector3D BottomCircleCenter
        {
            get { return this.vertices[0]; }
            set { this.vertices[0] = value; }
        }
        ...
        {
            get { return (this.TopCircleCenter - this.BottomCircleCenter).Magnitude; }
        }
        ...
        public Cylinder(Vector3D bottomCircleCenter, Vector3D topCircleCenter, double radius)
            :base(bottomCircleCenter, topCircleCenter)
        {
            this.Radius = radius;
        }
        ...
        public double GetArea()
        {
            return this.Height * 2 * Math.PI * this.Radius + 2 * new Circle(this.BottomCircleCenter, this.Radius).GetArea();
        }

        public double GetVolume()
        {
            return this.Height  * new Circle(this.BottomCircleCenter, this.Radius).GetArea();
        }
    }
    ...
    public class ExtendedFigureController : FigureController
    {
                ...
                case "cylinder":
                    {
                        Vector3D a = Vector3D.Parse(splitFigString[1]);
                        Vector3D b = Vector3D.Parse(splitFigString[2]);
                        double c = double.Parse(splitFigString[3]);
                        currentFigure = new Cylinder(a, b, c);
                        break;
                    }
            }
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        Console.WriteLine((currentFigure is IAreaMeasurable) ?
                            string.Format("{0:0.00}", (currentFigure as IAreaMeasurable).GetArea()) : "undefined");
                        break;
                    }
                case "volume":
                    {
                        Console.WriteLine((currentFigure is IVolumeMeasurable) ?
                            string.Format("{0:0.00}", (currentFigure as IVolumeMeasurable).GetVolume()) : "undefined");
                        break;
                    }
                case "normal":
                    {
                        Console.WriteLine((currentFigure is IFlat) ?
                            string.Format("{0:0.00}", (currentFigure as IFlat).GetNormal()) : "undefined");
                        break;
                    }
            }
